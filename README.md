...
How To Use: this program starts by asking the user if they would like to add a player, delete a player, or view all players on his fantasy football team. If the user adds a player, he will be prompted to provide information such as player name, position, their matchup strength for that specific week, their average points per game leading up to that week, and an subjective rating of the player's consistency. This will then output that players predicted score for the upcoming week, as well as a likely range that the player will fall into, with the upper bound and lower bound being determined by the player's consistency. this stores the player's information and predicted score in a list. it then prompts you to add/delete a player, or view team. deleting a player simply provides a numbered list of all players, and lets the user choose one to delete from the list. finally if the user chooses to view the team, it provides the full list of all the players entered, and allows the user to compare each player's prediction, and range of scores for each player, allowing the user to pick his best team that week. (Note: after entering real information for 14 players for a week of football, 12/14 players has a score in their predicted range)
